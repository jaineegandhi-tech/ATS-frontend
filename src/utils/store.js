...
const DEMO_CANDIDATES = [
  ...
  { id: 'CAND003', firstName: 'Carol', lastName: 'Williams', email: 'carol.williams@email.com', mobile: '+1-555-1003', location: 'Boston', appliedPosition: 'Marketing Manager', department: 'Marketing', employmentType: 'Full Time', status: 'Interview Scheduled', currentRound: 'HR Round', timeline: [{ action: 'Candidate Created', by: 'EMP006', at: new Date().toISOString() }], createdAt: new Date().toISOString(), createdBy: 'EMP006' },
  { id: 'CAND004', firstName: 'David', lastName: 'Brown', email: 'david.brown@email.com', mobile: '+1-555-1004', location: 'Chicago', appliedPosition: 'Sales Executive', department: 'Sales', employmentType: 'Full Time', status: 'Interview Scheduled', currentRound: 'HR Round', timeline: [{ action: 'Candidate Created', by: 'EMP006', at: new Date().toISOString() }], createdAt: new Date().toISOString(), createdBy: 'EMP006' },
  { id: 'CAND005', firstName: 'Emma', lastName: 'Davis', email: 'emma.davis@email.com', mobile: '+1-555-1005', location: 'Seattle', appliedPosition: 'Financial Analyst', department: 'Finance', employmentType: 'Full Time', status: 'Interview Scheduled', currentRound: 'HR Round', timeline: [{ action: 'Candidate Created', by: 'EMP006', at: new Date().toISOString() }], createdAt: new Date().toISOString(), createdBy: 'EMP006' },
  { id: 'CAND006', firstName: 'Frank', lastName: 'Miller', email: 'frank.miller@email.com', mobile: '+1-555-1006', location: 'Austin', appliedPosition: 'Operations Manager', department: 'Operations', employmentType: 'Full Time', status: 'Interview Scheduled', currentRound: 'HR Round', timeline: [{ action: 'Candidate Created', by: 'EMP007', at: new Date().toISOString() }], createdAt: new Date().toISOString(), createdBy: 'EMP007' },
...
export function initStore() {
  ...
  if (!localStorage.getItem(STORAGE_KEYS.CANDIDATES)) {
    ...
    // Also patch existing seed candidates that are missing createdBy
    const patched = candidates.map(c => {
      const seed = DEMO_CANDIDATES.find(s => s.id === c.id);
      if (seed && !c.createdBy && seed.createdBy) return { ...c, createdBy: seed.createdBy };
      return c;
    });
    if (missing.length || patched.some((c, i) => c !== candidates[i])) {
      localStorage.setItem(STORAGE_KEYS.CANDIDATES, JSON.stringify([...patched, ...missing]));
    }
  }
